...
import { Mail, Phone, MapPin, CreditCard, Lock, Calendar, Globe, Briefcase, User as UserIcon, AlertTriangle, Edit2 } from 'lucide-react';
...
export const Profile: React.FC = () => {
   ...
   const handleSaveProfile = async () => {
      const fname = (document.getElementById('edit-fname') as HTMLInputElement).value;
      const lname = (document.getElementById('edit-lname') as HTMLInputElement).value;
      ...
      // Private Info
      const marital = (document.getElementById('edit-marital') as HTMLInputElement).value;
      const nationality = (document.getElementById('edit-nationality') as HTMLInputElement).value;

      // File
      const fileInput = document.getElementById('edit-profile-pic') as HTMLInputElement;
      const file = fileInput?.files?.[0];

      try {
         if (user) {
            // Prepare data object, including new fields if backend supports them (it does check profile columns in controller)
            // Note: backend employeeController check 'phone', 'address' specifically in 'Update Profile table' block.
            // It doesn't check 'marital_status' or 'nationality' yet. I need to update backend controller to support these!
            // Wait, I should verify employeeController.js again.
            // It only checked phone and address.
            // checking... yes, only phone and address.
            // I need to add marital_status and nationality to backend controller too.
            ...
            if (profile) setProfile({ ...profile, phone, address, maritalStatus: marital, nationality: nationality });

            // In real app, we would reload profile picture if changed, but optimistic update is tricky for file content without reading it. 
            // We'll trust the reload or simple alert for now.
            setIsEditing(false);
            alert("Profile Updated!");
            // Trigger re-fetch or simple reload
   ...
   // Allow edit if it's own profile OR if current user is ADMIN
   const canEdit = currentUser?.id === user.id || currentUser?.role === Role.ADMIN;
   const isOwnProfile = currentUser?.id === user.id; // Still track strict ownership for Salary constraint
   ...
   return (
      <div className="max-w-5xl mx-auto space-y-6 animate-fadeIn">
         ...
         <div className="bg-white border border-slate-200 shadow-sm rounded-lg p-6 flex flex-col md:flex-row gap-6 items-center md:items-start relative">
            ...
            <div className="w-32 h-32 rounded-full border-4 border-slate-50 overflow-hidden shadow-sm flex-shrink-0 relative group">
               <img src={user.profilePictureUrl || `https://ui-avatars.com/api/?name=${user.firstName}+${user.lastName}`} className="w-full h-full object-cover" />

               {/* Upload Overlay in Edit Mode */}
               {isEditing && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                     <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" id="edit-profile-pic" accept="image/*" />
               ...
               {isEditing ? (
                  <div className="space-y-4 max-w-sm">
                     <div className="flex gap-2">
                        <input className="border rounded px-2 py-1 w-full" defaultValue={user.firstName} id="edit-fname" placeholder="First Name" />
                        <input className="border rounded px-2 py-1 w-full" defaultValue={user.lastName} id="edit-lname" placeholder="Last Name" />
                     </div>
                     <input className="border rounded px-2 py-1 w-full bg-slate-100 cursor-not-allowed" defaultValue={profile.jobPosition} disabled title="Job Position cannot be changed by user" />
                     <div className="flex flex-col gap-2 pt-2">
                        <input className="border rounded px-2 py-1 w-full text-sm" defaultValue={profile.phone} id="edit-phone" placeholder="Phone" />
                        <input className="border rounded px-2 py-1 w-full text-sm" defaultValue={profile.address} id="edit-address" placeholder="Address" />
                     </div>

                     {/* Private Info Edit */}
                     <div className="pt-2 border-t mt-2">
                        <label className="text-xs font-semibold text-slate-500 block mb-1">Private Info</label>
                        <div className="grid grid-cols-2 gap-2">
                           <input className="border rounded px-2 py-1 w-full text-sm" defaultValue={profile.maritalStatus} id="edit-marital" placeholder="Marital Status" />
                           <input className="border rounded px-2 py-1 w-full text-sm" defaultValue={profile.nationality} id="edit-nationality" placeholder="Nationality" />
                        </div>
                     </div>

                     <div className="flex gap-2 pt-2">
                        <button onClick={handleSaveProfile} className="bg-slate-900 text-white px-3 py-1 rounded text-sm hover:bg-slate-800">Save Changes</button>
                        <button onClick={() => setIsEditing(false)} className="bg-slate-100 text-slate-600 px-3 py-1 rounded text-sm hover:bg-slate-200">Cancel</button>
                     </div>
                  </div>
               ) : (
                  <>
                  ...
                  <div className="space-y-4">
                     ...
                     <button
                        onClick={() => {
                           ...
                           if (newPass) {
                              api.auth.resetPassword(user.id, newPass)
                                 .then(() => alert("Password updated successfully"))
                                 .catch(() => alert("Failed to update password"));
                           }
                        }}
                        className="w-full border border-slate-300 text-slate-600 py-2 rounded-lg hover:bg-slate-50 font-medium"
                     >
                        Reset Password
                     </button>
                  </div>
